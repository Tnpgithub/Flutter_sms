...
import 'table_calander_header.dart';
import 'table_calander_menager.dart';
...
class _TableCalendarState<T> extends State<TableCalendar<T>> {
                ...
                cell = Expanded(
                  child: Padding(
                    ...
                    child: Center(
                      child: Text(
                        weekNumber.toString(),
                        style: widget.calendarStyle.weekNumberTextStyle.copyWith(
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (!isDisabled) {
          ...
          Widget? markerWidget =
          widget.calendarBuilders.markerBuilder?.call(context, day, events as List<T>);
          ...
          if (events.isNotEmpty && markerWidget == null) {
            ...
            markerWidget = PositionedDirectional(
              ...
              child: Row(
                ...
                children: events
                    .take(1)
                    .map((event) => _buildSingleMarker(DateTime.now(), event, markerSize))
                    .toList(),
              ),
            );
          }
  ...
  Widget _buildSingleMarker(DateTime day, EventModel event, double markerSize) {
    return
